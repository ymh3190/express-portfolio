...
const naver = (req, res) => {
  const config = {
    ...
    redirect_uri:
      "http://expressportfolio-env.eba-nuampp4q.us-east-1.elasticbeanstalk.com/oauth/naver/callback",
    state: "RANDOM_STATE",
  };
  const params = new URLSearchParams(config).toString();
...
const kakao = (req, res) => {
  const config = {
    ...
    redirect_uri:
      "http://expressportfolio-env.eba-nuampp4q.us-east-1.elasticbeanstalk.com/oauth/kakao/callback",
    response_type: "code",
  };
  const params = new URLSearchParams(config).toString();
